...
if ($component->isDefaultMode())
{
	...
	// Подключаем CSS-стили для кнопки создания нового элемента
	Asset::getInstance()->addCss('/local/templates/bitrix/crm.field.element/main.edit/style.css');
	...
	// Выводим кнопку создания элемента в полях с типом привязки к элементам в CRM сущностях
	if (!$canCreateNewEntity && LayoutSettings::getCurrent()->isSliderEnabled()) {
		if (isset($arParams['userField']['USER_TYPE_ID']) && $arParams['userField']['USER_TYPE_ID'] === 'crm') {
